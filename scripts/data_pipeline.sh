...
python "$SCRIPT_DIR/process/build_futures_curve.py" --symbol $SYMBOL --intervals $COMMA_INTERVALS

# Step 2: Build market data for each interval
echo "Building market data..."
for interval in $INTERVALS; do
    echo "Processing interval: $interval"
    python "$SCRIPT_DIR/process/build_market.py" --symbol $SYMBOL --interval $interval
done
